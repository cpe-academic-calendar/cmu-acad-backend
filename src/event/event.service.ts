...
export class EventService {
    ...
    async autoGenerate(start_semester) {
        ...
        start_semester = new Date(start_semester).setHours(0, 0, 0, 0)
            ...
            if (event[i].reference_event) {
                ...
                if (event[i].ref_start == 'before') {
                    if (event[i].ref_end == 'before') {
                        ...
                        const day = new Date(event[con_index].start_date).getDate() - (event[i].num_weeks * 7 + event[i].num_days) + 1 //start date before 
